...
import { Plus, CheckCircle, ChevronLeft, ChevronRight, Filter, Clock, MapPin, BookOpen, User, Calendar, Loader2, Inbox, Trash2 } from 'lucide-react'
import { useNavigate } from '@tanstack/react-router'
import {
  getMyExchangeTransactions,
...
export function MyExchangeRequestsPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        setIsLoading(true)
        const myTxs = await getMyExchangeTransactions()
        setRequests(Array.isArray(myTxs) ? myTxs.map(mapTransactionToUi) : [])
      } catch (error) {
        console.error('Error loading data:', error)
        alert('Không thể tải dữ liệu yêu cầu đổi lớp.')
      } finally {
        setIsLoading(false)
      }
    }
  ...
  const handleDelete = async (requestId: string) => {
    if (!confirm('Bạn có chắc chắn muốn xóa yêu cầu đổi lớp này không?')) {
      return
    }

    try {
      setDeletingId(requestId)
      const transactionId = Number(requestId)
      if (!Number.isFinite(transactionId)) {
        throw new Error('ID yêu cầu không hợp lệ')
      }
      
      await deleteExchangeTransaction(transactionId)
      setRequests((prev) => prev.filter((r) => r.id !== requestId))
      alert('Xóa yêu cầu đổi lớp thành công')
    } catch (error) {
      console.error('Error deleting request:', error)
      alert(error instanceof Error ? error.message : 'Xóa yêu cầu đổi lớp thất bại')
    } finally {
      setDeletingId(null)
    }
  }
            ...
            <div className="flex items-center justify-between">
              <div>
                <h1 className='text-3xl font-bold tracking-tight flex items-center gap-3'>
                  ...
                  Yêu cầu đổi lớp của tôi
                </h1>
                <p className='text-muted-foreground text-base mt-2'>
                  Xem và quản lý các yêu cầu đổi lớp của bạn.
                </p>
              </div>
              <Button
                onClick={() => navigate({ to: '/exchange-request/create' })}
                className="gap-2"
                size="lg"
              >
                ...
                <CardContent className="text-center py-16">
                  ...
                  <p className="text-sm text-muted-foreground mb-4">
                    {filteredRequests.length === 0 && requests.length > 0
                      ...
                      : 'Bạn chưa có yêu cầu đổi lớp nào'}
                  </p>
                  <Button
                    onClick={() => navigate({ to: '/exchange-request/create' })}
                    className="gap-2"
...
export default MyExchangeRequestsPage
